...
class ValueResolver(tir.PyStmtExprMutator):
    ...
    def visit_buffer_load_(self, node):
        ...
        if not self.simplify_values:
            # Normalize only the lookup index: proving the declared metadata
            # contract is required even when the resulting address stays symbolic.
            index = analyzer.simplify(index)
        if not analyzer.can_prove(index >= 0) or not analyzer.can_prove(index < len(values)):
            return tir.BufferLoad(node.buffer, indices)
        ...
        return analyzer.simplify(result) if self.simplify_values else result
...
def metadata_loop(node):
    """Recognize bounded scalar setup; tensor loops and pipelines stay symbolic."""
    ...
    stores = [item for item in nodes if isinstance(item, tir.BufferStore)]
    return (
        bool(stores)
        and all(item.buffer.scope() == "local.var" for item in stores)
        and not any(
            isinstance(item, tir.For | tir.IfThenElse)
            or isinstance(item, tir.Evaluate) and isinstance(item.value, tir.Call)
            for item in nodes
        )
    )
