...
class AnalysisTrace:
    ...
    def record(self, name, snapshot):
        ...
        try:
            ...
            _LOG.warning("TileTune trace checkpoint %s failed: %s", name, error)
    ...
    def __exit__(self, exc_type, error, traceback):
        ...
        block = (
            f"===== TileTune analysis {uuid4().hex[:12]} (pid={os.getpid()}) =====\n\n"
            + "\n".join(self.entries)
        ...
        try:
            ...
            _LOG.warning("Cannot write TileTune trace to %s: %s", self.path, trace_error)
        return False
