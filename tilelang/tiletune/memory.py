...
def resident_warps_estimate(col, shared_bytes, device_limits):
    ...
    This intentionally omits register allocation so the gate remains separate
    from compiler-resource policy and the post-compile resource filter.
    """
    ...
    warps = (threads + max(warp, 1) - 1) // max(warp, 1)
    return {
