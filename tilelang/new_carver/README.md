# New Carver code review guide

New Carver reads each candidate's actual PrimFunc, recognizes its operation
...
them, see the [walkthrough](../../docs/new_carver_walkthrough.md).
...
| `CarverSession.elaborate` | Record analysis errors as `analysis_failed` and stop the candidate before lowering. |
...
Existing tests are in [testing/python/new_carver](../../testing/python/new_carver).
...
result = analyze_prim_func(
    func,
    CarverConfig(trace_path="/tmp/carver_trace.log", ranking_metric="pipeline_time", performance_model=profile),
    target=target,
    device_limits=limits,
)

# The autotuner labels each analysis block with its config index and arguments.
tuner.set_carver_args(True, trace_path="/tmp/carver_trace.log")
```

The same option works in the `carver={...}` dictionary on `@tilelang.autotune`.
...
```bash
python -m examples.gemm.example_gemm_new_carver_trace --output /tmp/gemm_trace.log
```

The [example](../../examples/gemm/example_gemm_new_carver_trace.py) traces an
