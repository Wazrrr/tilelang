...
class DefaultPolicy:
    ...
    def check_tile_shape_isvalid(self, td: TileDict) -> bool:
        ...
        for node in self.ordered_nodes:
            ...
            # Explicit fused schedules may recompute a broadcast producer in
            # each consumer tile. Automatic schedules retain one-to-one grids.
            if node_grid_size != td.grid_size and (
                not self.tags.get("allow_broadcast_recompute", False) or td.grid_size % node_grid_size
            ):
                return False
