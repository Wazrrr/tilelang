...
DEFAULT_MAX_EXTERNAL_BUSY_CORES = 2
...
def snapshot():
    ...
    # Linux load average is machine-wide, while the coordinator deliberately
    # narrows its own affinity after reserving worker CPUs. Compare like with
    # like here; contention inside the allocated CPU set is measured
    # independently by CpuMonitor.
    observation["host_cpu_count"] = os.cpu_count() or len(os.sched_getaffinity(0))
    return observation
...
def run_monitored(
    ...
    *,
    ...
    cpu_ids=None,
    pass_fds=(),
    max_external_busy_cores=DEFAULT_MAX_EXTERNAL_BUSY_CORES,
    ...
    """
    output = Path(output)
    if cpu_contention_policy not in ("reject", "record"):
        raise ValueError("cpu_contention_policy must be reject or record")
    ...
    cpu_telemetry = dict(samples=0, max_external_busy_cores=0.0, max_iowait_cores=0.0, max_steal_cores=0.0)
    if cpu_ids is not None:
        ...
        audit.update(
            cpu_ids=cpu_ids,
            max_external_busy_cores=max_external_busy_cores,
            max_iowait_cores=1,
            max_steal_cores=0.1,
    ...
    try:
        with (output / "gpu_observations.jsonl").open("a") as observations, (output / "worker.log").open("w") as log:
            while True:
                ...
                cpu_busy = cpu is not None and (
                    not cpu["ready"]
                    or cpu["external_busy_cores"] > max_external_busy_cores
                    or cpu["iowait_cores"] > 1
                    or cpu["steal_cores"] > 0.1
                )
                cpu_contention_samples = cpu_contention_samples + 1 if cpu_busy else 0
                if cpu is not None:
                    observed["cpu_observation"] = cpu
                    if cpu["ready"]:
                        cpu_telemetry["samples"] += 1
                        for source, destination in (
                            ("external_busy_cores", "max_external_busy_cores"),
                            ("iowait_cores", "max_iowait_cores"),
                            ("steal_cores", "max_steal_cores"),
                        ):
                            cpu_telemetry[destination] = max(cpu_telemetry[destination], cpu[source])
                ...
                if process is None:
                    ...
                    cpu_rejected = cpu_contention_policy == "reject" and (overloaded or cpu_busy)
                    if foreign or busy or delayed or cpu_rejected:
                        quiet_samples = 0
                        if not wait_idle:
                    ...
                    write_json(output / "monitor.json", audit)
                elif delayed or cpu_contention_policy == "reject" and (overloaded or cpu_contention_samples >= 2):
                    audit.update(
                    ...
                    success = "gpu_uncontended" if cpu_contention_policy == "record" else "uncontended"
                    audit.update(status=success if process.returncode == 0 else "worker_failed", exit_code=process.returncode)
        ...
        audit["cpu_telemetry"] = cpu_telemetry
        audit["cpu_contention_observed"] = (
            cpu_telemetry["max_external_busy_cores"] > max_external_busy_cores
            or cpu_telemetry["max_iowait_cores"] > 1
            or cpu_telemetry["max_steal_cores"] > 0.1
        )
        if process is None:
