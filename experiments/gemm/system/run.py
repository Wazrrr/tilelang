"""Measure advanced GEMM autotuning with each system optimization, or all variants."""
...
from experiments._common import add_gemm_arguments, add_run_arguments, device_info, prepare_run, select_configs, write_json
...
def main():
    ...
    add_run_arguments(parser, "experiments/results/gemm/system")
    ...
    if len(set(devices)) != len(devices) or any(d < 0 or d >= torch.cuda.device_count() for d in devices):
        parser.error("--benchmark-devices must name distinct visible CUDA devices")
    if args.variant in ("multi_gpu", "combined", "all") and len(devices) < 2:
        parser.error("this variant requires at least two --benchmark-devices")
    if args.variant == "all":
        run_all(args)
        return
    ...
    with set_autotune_inputs(inputs):
        tuner = (
            ...
            .set_compile_args(out_idx=[2], target="cuda", execution_backend="tvm_ffi")
            .set_profile_args(ref_prog=reference, backend=args.backend, cache_input_tensors=True, rtol=0.01, atol=0.01)
