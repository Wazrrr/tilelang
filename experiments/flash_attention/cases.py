...
def cases(holdout=False):
    ...
    return [
        Workload("attention_" + role, "attention", dict(batch=1, heads=4, sequence=n, dim=d, causal=causal), config_space="large")
...
def training_cases():
    ...
    return [
        Workload("attention_" + split, "attention", dict(batch=1, heads=2, sequence=n, dim=d, causal=c), config_space="large")
