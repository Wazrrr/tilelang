...
Space version 11 gives each final operation exactly one `expanded` pool. Each pool
...
`--smoke` selects one representative development case per family, preserving the grid. `--config-indices` explicitly
