...
def analyze_register_policy(pressure, config, device_limits=None, *, spill_allowance=0):
    ...
    physical_reasons = []
    launch_threads = ws.get("launch_threads") if known else (pressure.get("tile_liveness") or {}).get("computing_threads_estimate")
    ...
    demand = {
        ...
        "assumptions": [
            ...
            "logical accumulator demand is not physical allocation; strict user policies can reject its lower bound",
    ...
    if proven_exceeds and strict_demand:
        policy_reasons.append(f"proven tile demand {lower} exceeds demand limit {capacity} + allowance {allowance} = {threshold}")
    uncertainty = []
    if proven_exceeds or status == "exceeds_allowance":
        uncertainty.append("logical register demand exceeds the modeled allowance; compiler allocation and spill service are unresolved")
    reasons = physical_reasons + policy_reasons
    return {
