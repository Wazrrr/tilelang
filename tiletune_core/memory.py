...
def score_memory(accesses, grid_blocks, sm_count, pipeline_depth=1):
    ...
    score = _encode_memory_order(adjusted_byte_waves, pipeline_depth, event_waves) if adjusted_byte_waves is not None else None
    return {
        "metric": "memory",
        "score": score,
        ...
        "formula": (
            "65535 * adjusted_byte_waves * (adjusted_byte_waves + 1) // 2 + "
            "(65535 - pipeline_depth) * (adjusted_byte_waves + 1) + access_waves"
        ),
