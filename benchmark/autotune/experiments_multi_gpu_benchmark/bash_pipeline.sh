...
# Baseline: single-GPU benchmark mode + pipeline
CUDA_VISIBLE_DEVICES=5 python benchmark/autotune/run_gemm_autotune_e2e.py \
  --execution-backend auto \
  "${COMMON_ARGS[@]}" \
  --no-benchmark-multi-gpu \
  "${SHAPE_ARGS[@]}" \
  --csv "${RESULT_DIR}/auto_pipeline_baseline_single_gpu.csv"

# Multi-GPU benchmark mode + pipeline (same compile mode: grouped compile disabled)
CUDA_VISIBLE_DEVICES=5,6 python benchmark/autotune/run_gemm_autotune_e2e.py \
  --execution-backend auto \
  "${COMMON_ARGS[@]}" \
  --benchmark-multi-gpu \
  "${BENCHMARK_DEVICE_ARGS[@]}" \
  "${SHAPE_ARGS[@]}" \
  --csv "${RESULT_DIR}/auto_pipeline_benchmark_multi_gpu.csv"
