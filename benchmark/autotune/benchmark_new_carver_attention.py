...
from benchmark.autotune.benchmark_new_carver_gemm import ObservedTuner, snapshot
from examples.flash_attention.example_mha_new_carver import (
...
from tilelang.new_carver import load_device_profile
from tilelang.new_carver.device_profile import current_target, _identity
            ...
            def checkpoint(done):
                try:
                    for index, _, _, _ in done.result():
                        record = dict(self.carver_session.records[index] if self.carver_session else self.outcomes[index])
...
def main():
    ...
    if args.mode != "disabled":
        tuner.set_carver_args(
            True,
            mode=args.mode,
            report_path=str(args.output / "carver.json"),
