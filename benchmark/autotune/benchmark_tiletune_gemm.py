"""Reproducible cold-cache comparison of TileTune on a simple GEMM.
...
def main():
    ...
    if args.mode != "disabled":
        tuner.set_tiletune_args(
            True,
            mode=args.mode,
            report_path=str(run_dir / "tiletune.json"),
    ...
    summary = {
        ...
        "analysis_source_sha256": {
            path.relative_to("tilelang/tiletune").as_posix(): hashlib.sha256(path.read_bytes()).hexdigest()
            for path in sorted(Path("tilelang/tiletune").rglob("*.py"))
        },
        "gpu_before": before,
        "gpu_after": after,
        "wall_s": elapsed,
