...
from benchmark.autotune.benchmark_tiletune_attention import AttentionTuner, write
from benchmark.autotune.benchmark_tiletune_gemm import full_configs, snapshot
from benchmark.autotune.tiletune_experiment_utils import (
...
def freeze(root, profile, split, *, mode="report_only", group_size=1):
    ...
    paths = [
        *Path("tilelang/tiletune").rglob("*.py"),
        Path(__file__),
        Path("benchmark/autotune/benchmark_tiletune_attention.py"),
        Path("benchmark/autotune/benchmark_tiletune_gemm.py"),
        Path("benchmark/autotune/tiletune_experiment_utils.py"),
        ...
        with (root / (w["name"] + ".log")).open("w") as log:
            completed = subprocess.run(
                [
                    sys.executable,
                    "-m",
                    "benchmark.autotune.validate_tiletune_generalization",
                    "run",
                    "--root",
                    str(root),
...
def evaluate(root):
    ...
    for w in p["workloads"]:
        ...
        if s.get("tiletune") is None:
            ...
            continue
        records, measured = s["tiletune"]["configs"], s["configs"]
        ...
        results.append(
            dict(
                ...
                wall_seconds=s["wall_seconds"],
                stage_cost_percent=s["tiletune"]["stage_cost_percent"],
                error=s["error"],
...
def predict(root, profile_path, destination):
    ...
    for w in p["workloads"]:
        ...
        config = TileTuneConfig(enabled=True, device_limits=p["device_limits"], **settings(w, profile))
        records = []
        for i, c in enumerate(configs_for(w)):
            try:
