# New Carver: from PrimFunc to filtering, timing estimates, and benchmarking
...
The [code review guide](../tilelang/new_carver/README.md) maps the source into
...
## 1. What New Carver does

New Carver analyzes each candidate's **actual, elaborated TileLang PrimFunc**.
...
That native connection is central: New Carver does not reconstruct a hypothetical
...
Sources: [configuration](../tilelang/new_carver/config.py),
[module orchestration](../tilelang/new_carver/engine.py),
[tuner integration](../tilelang/autotuner/tuner.py#L1336),
[session reporting](../tilelang/new_carver/runtime.py#L171).
...
`report_only` keeps candidates through Carver's gates; it does not turn a
...
In the integrated path, `CarverSession.elaborate()` times elaboration and analysis
...
Sources: [GEMM elaboration example](../examples/gemm_fp8/example_gemm_fp8_new_carver.py#L34),
[attention elaboration example](../examples/flash_attention/example_mha_new_carver.py#L38),
[session elaboration](../tilelang/new_carver/runtime.py#L115),
...
```text
tl.new_carver.ParseOperator(call, enclosing_block_annotations)
tl.new_carver.GetAccessRegions(tile_operator)
```
...
Sources: [collector](../tilelang/new_carver/analysis.py#L91),
...
Source: [operation records and traversal](../tilelang/new_carver/analysis.py#L62).
...
Sources: [input mapping](../tilelang/new_carver/analysis.py#L377),
[propagation](../tilelang/new_carver/analysis.py#L490),
[analysis entry point](../tilelang/new_carver/analysis.py#L549).
...
Sources: [GEMM family](../tilelang/new_carver/families/gemm.py),
[attention family](../tilelang/new_carver/families/attention.py),
[common execution order](../tilelang/new_carver/engine.py).
...
`tl.new_carver.ClassifyProducerCopy`, which uses the compiler's copy classifier.
...
Sources: [WS predictor](../tilelang/new_carver/warp_specialization.py),
[native copy classification adapter](../src/cuda/op/copy_analysis.cc#L859),
[device-profile targets](../tilelang/new_carver/device_profile.py#L24).
...
Sources: [initial register-analysis flow](../tilelang/new_carver/register_pressure.py),
[accumulator evidence, ownership and lower bound](../tilelang/new_carver/register_accumulator.py).

Allocation descriptions are separate in
[register_storage.py](../tilelang/new_carver/register_storage.py). Their size and
...
Source: [live-tile analysis](../tilelang/new_carver/liveness.py).
...
The attention allowance defaults to zero in `CarverConfig`. The attention
...
register-policy decision. `CarverSession` then enforces it in `reject` mode.
`report_only` records `would_reject` but continues.

Sources: [architecture/user budget](../tilelang/new_carver/budget.py),
[register policy](../tilelang/new_carver/register_policy.py),
[decision enforcement](../tilelang/new_carver/runtime.py#L123).
...
Source: [memory analysis](../tilelang/new_carver/memory.py).
...
Source: [shared-storage planning](../tilelang/new_carver/shared_storage.py).
...
Sources: [occupancy and waves](../tilelang/new_carver/waves.py),
[device-limit queries](../tilelang/new_carver/cost.py#L21).
...
Sources: [profile construction and caching](../tilelang/new_carver/device_profile.py),
[primitive kernels](../tilelang/new_carver/device_probes.py).
...
Sources: [operation work](../tilelang/new_carver/operation_work.py),
[service-time calculation](../tilelang/new_carver/service.py),
[reduction mapping and work](../tilelang/new_carver/reduction.py).
...
Source: [buffer collection and recurrence](../tilelang/new_carver/tile_schedule.py).
...
Source: [serial/overlapping timing branches](../tilelang/new_carver/pipeline.py).
...
Sources: [CTA work and dispatch](../tilelang/new_carver/cta_work.py),
[grid-score integration](../tilelang/new_carver/ranking.py#L7).
...
Sources: [metric and sorting logic](../tilelang/new_carver/ranking.py),
[benchmark submission](../tilelang/autotuner/tuner.py#L1640),
[report finalization](../tilelang/new_carver/runtime.py#L171).
...
Sources: [compiler resource checks](../tilelang/new_carver/runtime.py#L15),
...
The strength of New Carver is the connection between **native operator semantics,
