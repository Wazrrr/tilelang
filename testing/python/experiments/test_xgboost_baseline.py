...
def test_orchestration_edits_do_not_change_kernel_domain(tmp_path):
    from experiments.xgboost.data import context_from_experiment, domain

    path = write_run(tmp_path / "train", 16)
    experiment = json.loads((path / "experiment.json").read_text())
    sources = experiment["source_sha256"]
    for name in (
        "experiments/common/study.py",
        "experiments/common/brute_force.py",
        "experiments/gemm/census.py",
        "experiments/gemm/system/run.py",
    ):
        sources[name] = "initial orchestration"
    for name in ("experiments/common/run.py", "experiments/common/execution.py", "experiments/utils/cli.py"):
        sources[name] = "initial execution"
    original = domain(context_from_experiment(experiment))
    for name in tuple(sources):
        if sources[name] == "initial orchestration":
            sources[name] = "changed orchestration"
    assert domain(context_from_experiment(experiment)) == original
    for name in ("experiments/common/run.py", "experiments/common/execution.py", "experiments/utils/cli.py"):
        sources[name] = "changed execution"
        assert domain(context_from_experiment(experiment)) != original
        sources[name] = "initial execution"
...
def _build_with_bad_rows(block_rows, threads):
    ...
    return make_case(Workload("gemm", "gemm", dict(m=16, n=128, k=128, transpose_b=True))).build(
        block_M=32 * block_rows, block_N=64, block_K=32, thread_num=threads, num_stages=0, enable_rasteration=False
    )
