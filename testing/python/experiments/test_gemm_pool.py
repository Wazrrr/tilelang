...
def test_gemm_pool_is_complete_and_identical_across_targets(target):
    ...
    assert space["candidate_count"] == len(set(space["config_ids"])) == 1920
...
def test_final_study_uses_all_1920_and_smoke_records_a_subset():
    ...
    assert all(len(c["indices"]) == 1920 for c in final["subsets"]["hopper"].values())
