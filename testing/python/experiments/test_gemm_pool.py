...
def test_gemm_pool_is_complete_and_identical_across_targets(target):
    ...
    assert space["candidate_count"] == len(set(space["config_ids"])) == 576
...
def test_final_study_uses_all_576_and_smoke_records_a_subset():
    ...
    assert all(len(c["indices"]) == 576 for c in final["subsets"]["hopper"].values())
