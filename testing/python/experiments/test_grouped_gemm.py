...
def test_pool_identity_subsets_and_system_variants():
    ...
    indices = [space["configs"].index(dict(CONFIG, block_N=128, threads=256)), space["configs"].index(CONFIG)]
    device = Device("hopper", TARGETS["hopper"])
    subset = [space["configs"][i] for i in indices]
    ...
    plan = system_plan("grouped_gemm", indices=indices)
    assert len(plan) == 25
    assert {row["variant"] for row in plan} == set(VARIANTS)
    assert all(row["indices"] == indices for row in plan)
