...
COUNTS = {
    "attention": 480,
    "kda_chunk_intra_token_parallel": 234,
    "gemm_fp8": 2304,
    "grouped_gemm": 576,
}
...
def test_every_example_config_and_explicit_default_is_included():
    ...
    assert len(kda_configs()) == 96
    # The KDA pool is fixed at num_stages=0 because deeper pipelines can hang
    # on Ampere, so only the example's non-pipelined configs are retained.
    assert all(c in kda for c in kda_configs() if c["num_stages"] == 0)
    assert dict(block_H=1, num_stages=0, threads=128, block_DK=128) in kda
    assert dict(block_H=16, num_stages=0, threads=256, block_DK=128) in kda
...
def test_retired_kda_operations_are_not_silently_substituted():
    for op in ("kda_recurrent", "kda_chunk_o"):
        with pytest.raises(ValueError, match="Unknown operation"):
            Workload("retired", op, dict(batch=1, heads=1, sequence=64, dim=64, chunk_size=64, sub_chunk_size=16))
