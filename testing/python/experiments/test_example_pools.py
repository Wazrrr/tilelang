...
COUNTS = {"attention": 320, "kda_chunk_o": 720, "gemm_fp8": 72, "grouped_gemm": 192}
...
def test_every_example_config_and_explicit_default_is_included():
    ...
    assert len(fp8) == 72
    assert all(c["block_K"] == 128 for c in fp8)
    assert dict(block_M=64, block_N=128, block_K=128, num_stages=2, threads=256) in fp8
