"""A failed shared build must not discard valid configs or undo policy rejection."""
...
def indices(mod):
    return sorted({int(re.search(r"_gc_(\d+)", str(gv.name_hint))[1]) for gv in mod.functions})


@pytest.fixture
def backend(monkeypatch):
    """Inject compiler failures while exercising the real grouping and policy code."""
    ...
    def device_codegen(mod, target):
        ...
        if set(group) & state.bad_device:
            raise RuntimeError(f"invalid device configs {sorted(set(group) & state.bad_device)}")
        for gv in mod.functions:
            name = str(gv.name_hint)
            idx = int(name.rsplit("_gc_", 1)[1])
            spills = 4 if idx in state.rejected else 0
            record_usage(
                parse_ptxas_output(f"""ptxas info : Function properties for {name}
    0 bytes stack frame, {spills} bytes spill stores, 0 bytes spill loads
...
""")
    ...
    def host_codegen(mod, target_host, *, target):
        ...
        if set(group) & state.bad_host:
            raise RuntimeError(f"invalid host configs {sorted(set(group) & state.bad_host)}")
...
def compile_eight(backend, *, session=None):
    configs = [dict(variant=i) for i in range(8)]
    results = gc.compile_grouped_unit_tvm_ffi(
        list(enumerate(configs)),
        CompileArgs(target=tvm.target.Target({"kind": "cuda", "arch": "sm_90a"}), execution_backend="tvm_ffi"),
        backend.elaborate,
    ...
    assert Counter(backend.lowered) == Counter(set(range(8)) - backend.bad_elaborate)
...
def test_shared_failure_isolated_to_invalid_configs(backend, stage, bad):
    ...
    results = compile_eight(backend)
    for idx, (kernel, error) in results.items():
        assert (kernel is None) == (idx in bad)
        if idx in bad:
            assert str(error) == f"invalid {stage} configs [{idx}]"
        else:
            assert error is None
    # A binary split is bounded even when every singleton fails.
...
@pytest.mark.parametrize("rejected", [{2}, {2, 4, 5, 6, 7}])
def test_fallback_preserves_lowering_failures_and_post_compile_rejection(backend, monkeypatch, rejected):
    backend.bad_elaborate = {0}
    backend.bad_lower = {1}
    backend.rejected = rejected
    backend.bad_host = {3}
    configs = [dict(variant=i) for i in range(8)]
    session = TileTuneSession(
        TileTuneConfig(enabled=True, mode="report_only", post_compile_policy=dict(mode="reject", max_spill_bytes=0)),
        configs,
        target={"kind": "cuda", "arch": "sm_90a"},
    )
    # Ranking is already frozen. Exercise the actual post-compile policy here;
    # analysis and top-alpha integration are covered by the GPU test below.
    ...
    results = compile_eight(backend, session=session)
    ...
    assert all(results[idx][0] is not None for idx in set(range(4, 8)) - rejected)
    for idx in rejected:
    ...
    assert all(session.records[idx]["grouped_compile_fallbacks"] for idx in set(range(3, 8)) - rejected)
...
def test_compile_cost_includes_failed_shared_attempts(backend, monkeypatch, stage):
    ...
    session = TileTuneSession(
        TileTuneConfig(enabled=True, mode="report_only"),
        [dict(variant=i) for i in range(8)],
        target={"kind": "cuda", "arch": "sm_90a"},
    )
    monkeypatch.setattr(session, "elaborate", lambda idx, config, elaborate, **kwargs: elaborate(**config))
    compile_eight(backend, session=session)
...
def copy_kernel(block=32, threads=32):
    @T.prim_func
...
def test_gpu_pipeline_alpha_group_eight(monkeypatch, tmp_path, inject_failure):
    ...
    def codegen(mod, target):
        if inject_failure and 3 in indices(mod):
    ...
    tuner = (
        ...
        .set_tiletune_args(
            TileTuneConfig(
                ...
                post_compile_policy=dict(mode="reject", max_spill_bytes=0, max_local_bytes=0),
    ...
    assert len(lowered) == len(set(lowered)) == 8
