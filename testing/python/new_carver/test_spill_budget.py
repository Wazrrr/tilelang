...
def test_tuner_resolves_physical_limits_without_ranking(monkeypatch):
    ...
    monkeypatch.setattr("tilelang.new_carver.query_device_limits", queried)
    tuner = AutoTuner(gemm, [{}]).set_compile_args(target=TARGET, execution_backend="tvm_ffi").set_carver_args(True, ranking=False)
