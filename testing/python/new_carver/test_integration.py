...
def test_gpu_exhaustive(tmp_path, grouped, mode):
    ...
    if mode:
        tuner.set_carver_args(True, mode=mode, report_path=str(tmp_path / "report.json"))
...
def test_architecture_budget_controls_lowering(monkeypatch, tmp_path, grouped, mode):
    ...
    session = CarverSession(tilelang.CarverConfig(enabled=True, mode=mode, report_path=str(tmp_path / "report.json")), configs)
    reached = []
    ...
    for unit in [items] if grouped else [[item] for item in items]:
        results.extend(
            gc.compile_grouped_unit_tvm_ffi(
                unit, CompileArgs(target=Target({"kind": "cuda", "arch": "sm_90a"})), kernel, carver_session=session
            )
        )
...
def test_rejection_never_lowers_and_failure_attribution(monkeypatch, tmp_path, grouped):
    ...
    session = CarverSession(tilelang.new_carver.CarverConfig(enabled=True, register_cap=1), [{"block": 32}, {"block": -1}])
    calls = []
    ...
    results = []
    for unit in units:
        results.extend(gc.compile_grouped_unit_tvm_ffi(unit, CompileArgs(target=Target("cuda")), elaborate, carver_session=session))
    assert len(results) == 2
    assert calls == [32, -1]
...
def test_gpu_mma_wgmma_and_function_settings(tmp_path):
    ...
    for mode in ("report_only", "reject"):
        ...
        session = CarverSession(tilelang.CarverConfig(enabled=True, mode=mode), configs)
...
def test_decorator_binding_and_fresh_report(monkeypatch, tmp_path):
    tuned = tilelang.autotune(
        ...
        carver={"mode": "report_only", "report_path": str(tmp_path / "fresh.json")},
        warmup=1,
        rep=1,
    ...
    assert all(r["status"] == "benchmarked" for r in tuned.carver_report["configs"])
    assert all(r["pressure"]["budget"] == 255 for r in tuned.carver_report["configs"])
...
def test_analysis_failure_still_reaches_lowering(monkeypatch):
    ...
    session = CarverSession(tilelang.CarverConfig(enabled=True), [{"block": 32}, {"block": 64}])
    reached = []
    ...
    monkeypatch.setattr("tilelang.new_carver.runtime.analyze_prim_func", unknown)
    monkeypatch.setattr(gc, "lower_to_host_device_ir", lower)
...
def test_gpu_ws_policy_matches_compiled_launch_and_function_settings(grouped):
    ...
    session = CarverSession(tilelang.CarverConfig(enabled=True, mode="report_only"), configs)
    calls = []
...
def test_ws_analysis_receives_effective_compile_override(monkeypatch):
    ...
    session = CarverSession(tilelang.CarverConfig(enabled=True), [{}])
