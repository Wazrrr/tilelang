...
from tilelang.new_carver import CarverConfig, check_compiler_resources
from tilelang.contrib.cuda_resource_info import KernelResourceUsage, parse_ptxas_output
from tilelang.new_carver.budget import resolve_register_budget
...
def test_architecture_register_budget(arch, user_cap, expected):
    config = CarverConfig(register_cap=user_cap)
...
def test_unknown_architecture_keeps_user_budget(target):
    budget = resolve_register_budget(CarverConfig(), target)
    assert budget["budget"] is None
    assert budget["hardware_register_cap"] is None
    assert budget["budget_source"] is None
    budget = resolve_register_budget(CarverConfig(register_cap=128), target)
...
def test_register_budget_uses_target_scope_without_overriding_explicit_target():
    with Target({"kind": "cuda", "arch": "sm_90a"}):
        assert resolve_register_budget(CarverConfig())["budget"] == 255
        assert resolve_register_budget(CarverConfig(), "llvm")["budget"] is None
...
def test_settings_cache_identity():
    ...
    disabled = key()
    tuner.set_carver_args(True)
    enabled = key()
    assert disabled != enabled
    tuner.set_carver_args(True, max_spill_bytes=None, max_local_bytes=None)
    assert key() != enabled
    tuner.set_carver_args(True, max_spill_bytes=0, max_local_bytes=0)
    assert key() == enabled
...
def test_conflicts_and_hooks():
    tuner = AutoTuner(kernel, [{"block": 32}]).set_compile_args(target="cuda", execution_backend="tvm_ffi").set_carver_args(True)
