...
def test_every_final_workload_has_a_scored_native_configuration(workload):
    ...
    rates = dict(
        PROFILE,
        gemm_signature=dict(instruction="cuda.wgmma", a_dtype=workload.dtype, b_dtype=workload.dtype, accum_dtype="float32"),
        gemm_instruction_rates={"cuda.mma": dict(gemm_flops_per_cycle=2048)},
...
def test_kda_tails_count_both_products_padded_compute_and_masked_traffic():
    ...
    case = make_kda(Workload("tails", "kda_chunk_o", dict(batch=1, heads=4, sequence=768, dim=96, value_dim=80, chunk_size=48)))
