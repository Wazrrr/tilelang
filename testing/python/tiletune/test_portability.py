...
def test_experiment_grids_and_unsupported_workloads():
    from benchmark.autotune.validate_tiletune_generalization import SPLITS, configs_for, unsupported_reason
...
def test_experiment_freeze_includes_family_sources(tmp_path, monkeypatch):
    from benchmark.autotune import validate_tiletune_generalization as experiment
...
def test_unknown_scores_do_not_claim_top_k_performance():
    from benchmark.autotune.tiletune_experiment_utils import evaluate_ranking
    from tilelang.tiletune import rank_records
...
def test_gpu_portable_experiment_roundtrip(tmp_path, monkeypatch, mode, group_size):
    ...
    for w in workloads:
        ...
        assert bool(summary["tiletune"]) == (mode != "disabled")
        timing = (root / w["name"] / "timings.tsv").read_text()
        assert "duration_ms" in timing
        if mode != "disabled":
            assert "tiletune.analysis" in timing
