...
def test_fp8_example_uses_actual_ir_and_fp32_accumulation():
    from examples.gemm_fp8.example_gemm_fp8_tiletune import get_configs, make_gemm
    ...
    for unit in [items] if grouped else [[item] for item in items]:
        results.extend(
            compile_grouped_unit_tvm_ffi(
                unit, CompileArgs(target=Target(TARGET), execution_backend="tvm_ffi"), elaborate, tiletune_session=session
            )
        )
