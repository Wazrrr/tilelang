...
from tilelang.tiletune.trace import AnalysisTrace
...
def test_trace_write_failure_does_not_change_decisions(tmp_path, capfd):
    ...
    assert before == after
    assert "Cannot write TileTune trace" in capfd.readouterr().out
...
def test_session_trace_labels_candidates_and_appends(tmp_path):
    ...
    session = TileTuneSession(TileTuneConfig(trace_path=str(path)), configs, target=TARGET, device_limits=LIMITS)
    for index, settings in enumerate(configs):
        session.elaborate(index, settings, gemm)
    blocks = path.read_text().split("===== TileTune analysis ")[1:]
    assert len(blocks) == 2
...
def test_invalid_trace_path(value):
    with pytest.raises(ValueError, match="trace_path"):
        TileTuneConfig(trace_path=value)


def test_trace_path_does_not_change_cache_identity():
    assert TileTuneConfig(trace_path="trace.log").to_cache_key_dict() == TileTuneConfig().to_cache_key_dict()
...
def test_decorator_repeats_analysis_when_trace_is_requested(tmp_path, monkeypatch):
    ...
    def run(self, *args, **kwargs):
        ...
        analyze_prim_func(func, self.tiletune_args, target=TARGET, device_limits=LIMITS)
        calls.append(True)
        return SimpleNamespace(kernel=sentinel, config={})

    monkeypatch.setattr(AutoTuner, "run", run)
    tuned = tilelang.autotune(configs=[{"stages": 2}], tiletune={"trace_path": str(path)})(
...
def test_trace_bypasses_autotuner_result_cache(tmp_path, monkeypatch, trace_enabled):
    ...
    tuner.set_tiletune_args(True, device_limits=LIMITS, trace_path=str(tmp_path / "trace.log") if trace_enabled else None)
